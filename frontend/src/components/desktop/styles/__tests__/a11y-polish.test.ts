 ...
 *   3. SettingsView toggle 按钮(深色模式 / 思考过程)有 aria-pressed
...
describe("Task 6: a11y 契约锁", () => {
  ...
  it("SettingsView 深色模式 toggle 必须有 aria-pressed", () => {
    expect(SETTINGS).toMatch(/onClick=\{handleToggleDarkMode\}[\s\S]*?aria-pressed=\{darkMode\}/);
  });

  it("SettingsView 思考过程 toggle 必须有 aria-pressed", () => {
    expect(SETTINGS).toMatch(/onClick=\{\(\) => setShowThinking\(!showThinking\)\}[\s\S]*?aria-pressed=\{showThinking\}/);
  });
