 ...
 *   3. PreferencesDrawer toggle 按钮(深色模式 / 思考过程)有 aria-pressed
 *      (第十三轮 2026-07-17:SettingsView 已合并到 PreferencesDrawer 通用 tab)
...
const PREFERENCES = readFileSync(
  resolve(HERE, "../../PreferencesDrawer.tsx"),
  "utf8"
);
...
describe("Task 6: a11y 契约锁", () => {
  ...
  it("PreferencesDrawer 深色模式 toggle 必须有 aria-pressed", () => {
    expect(PREFERENCES).toMatch(/onClick=\{handleToggleDarkMode\}[\s\S]*?aria-pressed=\{darkMode\}/);
  });

  it("PreferencesDrawer 思考过程 toggle 必须有 aria-pressed", () => {
