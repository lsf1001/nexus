...
| `ws-auth-subprotocol.spec.ts` | WS 鉴权协议契约（nxv1-* 子协议） |

### 10 条 user-journey spec（`frontend/e2e/journey/`）
...
| `journey-quick-prompts-and-history.spec.ts` | QUICK_PROMPTS + Sidebar 历史切换 | mock |
...
| `journey-redesign.spec.ts` | UI 旅程合集（hero / 新对话 / 多轮 / 工具 / 思考折叠 / 搜索 / 模型切换 / 主题 / 微信 / 历史） | 真 |
...
- **聊天消息里绝对路径 click-to-open + 图片内联缩略图**（2026-07-14 加）
  ...
  点击直达 Preview/Finder 与 file→file 缩略图加载都是浏览器 / WKWebView
  原生行为（file 协议 → macOS handler，file→file 默认 allow），
