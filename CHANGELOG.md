...
参考 Claude Desktop 把 agent 产物(代码 / Markdown / SVG / HTML)从消息流
独立到右栏"作品"面板,默认折叠态行为兼容旧两栏:

- `ArtifactsPanel` 右栏容器:head(filename + counter + close) + tabs(Code /
  Md / SVG / HTML,带 count badge) + body(渲染器) + foot(meta);空态用 📄 +
  "还没有产物"
- 4 个渲染器:`CodeRenderer`(highlight.js + 行号,18 种语言)、`MarkdownRenderer`
  (react-markdown + remark-gfm)、`SvgRenderer`(提取 `<svg>` inlined)、
  `HtmlRenderer`(`sandbox=allow-scripts` iframe 沙箱,自动 wrap fragment)
- `pushArtifact` store 扩展:`activeArtifactId` + `artifactsCollapsed` 字段;
  push 时自动激活 + 展开;`createdAt` 改为可选(向后兼容旧 caller)
- `ToolCallCard` 联动:file-class 工具(edit_file / write_md / write_html /
  draw_diagram 等)+ result ≥ 30 字符 → 显示"→ 在右侧查看"链接,点击触发
  pushArtifact(右侧自动展开并定位到该产物)
- `ShellLayout` 三栏 CSS grid:`260px minmax(0,760px) minmax(0,1fr)`,折叠态
  回退两栏;`@media (max-width: 768px)` 折叠 sidebar + artifacts,主区占满
- 快捷键 `⌘\` / `Ctrl+\` 折叠/展开右栏(useGlobalShortcuts 扩展)
- 新增 CSS:`components/desktop/styles/artifacts.css`(428 行,零新增变量,全
  复用 index.css tokens);`index.css` 中旧两列 grid 移除
- 渲染器测试 14 个 + ToolCallCard 联动测试 6 个 + 快捷键测试 3 个,全过

行为兼容:首次启动右栏仍折叠(行为同旧两栏);只有 ToolCallCard 联动 / ⌘\ 才展开。
...
#### Fixed — WechatAssistantView chat-status-bar 必须置顶 (2026-07-15)

系统性测微信通道所有交互时发现:chat-status-bar 出现在 `bind-card` 之下
(y ≈ 161 而非 y=0)。根因 — `ChannelViewBase` 套在 `chat-area-wrap` 外层,
`bind-card` + `ChannelInbox` 先渲染把 `chat-status-bar` 挤到下面,
**macOS chrome 顶部 36px drag 区域被顶掉,用户在窗口顶部抓不住拖窗**。

修法:`WechatAssistantView.tsx` 结构重排 — `chat-area-wrap` 提到最外层,
`chat-status-bar` 作为直接子元素顶部 y=0 渲染,`ChannelViewBase` 只包裹
`wechat-copy-inline` 这部分 brand-copy children(`bind-card` + `inbox`
由 `ChannelViewBase` 自带,在 `chat-status-bar` 之下)。

- **`frontend/src/components/desktop/WechatAssistantView.tsx`**:结构重排
  (`chat-area-wrap` → `header.chat-status-bar` → `ChannelViewBase` 含
  `wechat-copy-inline`)
- **`frontend/src/components/desktop/__tests__/WechatAssistantView.test.tsx`**:
  新增 7 个结构契约锁测试 — `chat-area-wrap` 是最外层 / status-bar 是
  `chat-area-wrap` 直接子(`ChannelViewBase` 内不允许有) / `bind-card` 顺序在
  status-bar 之后(`compareDocumentPosition`) / `wechat-copy-inline` 在
  `ChannelViewBase` 的 `.channel-children` 内 / onBack 缺省不渲染按钮 /
  onBack 提供渲染 "← 返回聊天" / status-bar 带 `data-tauri-drag-region`

测试结果:vitest 85/85 PASS(含 7 个新锁测试),pytest 926/926 PASS。

#### Changed — Dark 模式调色为宫崎骏森林绿 (2026-07-15)

用户截图反馈:DMG 深色模式整个变冷黑 + teal 蓝绿,品牌"森林绿 + 米色"完全消失。

根因:`tokens.css:51-67` 的 dark 模式 token 把 `--forest` 写成 `#25a4be` teal 蓝绿
(冷色调 + 高饱和蓝),与 light 模式 `#0f6e87` 宫崎骏森林绿脱钩;`--canvas` 用
`#0b111b` 偏冷黑底。`index.css:26-31` 的 `prefers-color-scheme: dark` 兜底也走
`#0f0f12` 纯冷黑,即便 JS 加载前 body 也被染色成冷色。

修法:dark token 整体调成宫崎骏森林绿族,与 light 模式保持品牌识别度:

- **`--canvas`**: `#0b111b` → `#0d1f17`(深森林底,G > B)
- **`--paper`**: `#111a27` → `#142a1e`(深苔藓)
- **`--paper-soft`**: `#172231` → `#1c3528`
- **`--forest`**: `#25a4be` → `#4a8a6f`(宫崎骏森林绿,G 主通道 / B/R ≈ 1.5)
- **`--forest-2`**: `#43b8cf` → `#6ba989`(hover/active 浅一档)
- **`--forest-soft`**: `#112e38` → `#1a3328`
...
详见原计划（已清理；e2e journey 套件决策可追溯 Git 历史）。
...
详见原 spec（已清理；e2e journey 套件结构与原则可参考当前 `frontend/e2e/README.md`）。
...
> 详细计划见原 `docs/superpowers/plans/2026-06-19-intent-recognition-routing.md`（已清理；当前实现见 `nexus/backend/intent/router.py`）。
...
> 详细计划见原 `docs/superpowers/plans/2026-06-20-observability-subsystem.md`（已清理；当前实现见 `nexus/backend/observability/`），运维文档见 `docs/operations/logging.md`。
...
> 详细计划与进度见 `docs/superpowers/plans/` + `docs/superpowers/progress.md`（已清理；当前质量门运维见 `docs/operations/quality.md`）。
...
- 原 `docs/superpowers/progress.md`：Phase 1+2 全部完成 + 真环境验收 4 条全过（进度文件已清理；当前事实见 `docs/operations/quality.md`）
...
验证:原 `docs/screenshots/2026-07-15-v7-dmg-dark.png` 真桌面截图(已清理)。puppeteer `getComputedStyle` 验过:dark 模式下 `.composer { background: rgb(53,99,73) }` = `#356349` = `paper-soft`,`.nexus-desktop` 背景 `rgb(33,74,53)` = canvas。
...
### [1.1.1] — 2026-07-15 第八轮 SettingsView + WechatAssistantView + SetupView 对齐 Claude Desktop 形态

#### Changed — SettingsView / WechatAssistantView / SetupView 改用同一 chat-area-wrap 模板

第六轮只重构了 ChatView(主聊天)+ 清理 dead code,但 SettingsView 和 WechatAssistantView 仍停留在"卡片 + 双栏 + 绝对定位返回按钮"的旧形态,跟 ChatView 不一致。本轮把三个子视图全部对齐到 ChatView 同源结构:

```
<div className="chat-area-wrap">                ← flex column,跟 ChatView/Settings/Wechat/Setup 共享
  <header className="chat-status-bar">         ← 36px 顶条,左标题 / 右 .chat-status-action 返回按钮
    <span className="chat-status-topic">设置</span>
    <button className="chat-status-action">← 返回聊天</button>
  </header>
  <div className="settings-list">...</div>      ← Settings:行+分割线扁平列表
  <section className="wechat-copy-inline">...</>  ← Wechat:brand-copy 单列左对齐
</div>
```

#### 关键改动

- **`SettingsView.tsx`**:去掉 `<section className="settings-view">` + `.settings-header` 绝对定位返回按钮,改用 `.chat-area-wrap` + 36px status bar(左"设置",右"← 返回聊天")
- **`WechatAssistantView.tsx`**:去掉双栏 grid `<section className="wechat-view">` (0.72fr / 1.28fr) + `.wechat-mark / .wechat-copy / .wechat-benefits` 散落样式,改成单列 `.wechat-copy-inline`(左 60px padding 与 settings/setup 起手对齐)
- **`SetupView.tsx`**:虽然 `.setup-card` 第六轮已经是 transparent 无 box,但 JSX 仍用 `<section className="setup-view">` 旧结构。本轮改为 `.chat-area-wrap` + 36px status bar "连接你的模型"
- **`shell.css`**:新增 `.chat-status-bar .chat-status-action` 36px 顶条右侧返回按钮样式(border-line-paper 配色 + hover bg,跟 status pill 同高)
- **`views.css`**:
  - 删 `.settings-view` / `.wechat-view` (双栏 grid + border-right)/ `.settings-header / .wechat-header` 浮层 / `.back-btn` 一整套
  - `.setting-row` 去卡片(`border + radius + shadow + paper bg`)改 `border-bottom: 1px var(--line)` 扁平分行,末行无分割线避免双线
  - `.settings-list` 去 `justify-self: center / align-self: center` 居中(改 max-width 720px 左对齐)
  - `.setup-view` 改为内层 flex column(left 60px padding),作为 chat-area-wrap 的内容槽
  - 新增 `.wechat-copy-inline` 单列 flex 容器

#### WHY 单源结构

- macOS 系统设置就是"行 + 分割线 + 右侧 toggle",没有任何圆角卡片;我们的 settings 也该跟 macOS 一样扁平
- 微信通道的"扫码绑定"是动作,brand copy 是说明;放在同一列上下走,比塞进双栏 grid 视觉更安静(原 `.wechat-copy` + border-right + 0.72fr/1.28fr = 两栏对称,主区内容反而被两栏"挤")
- 三个 view 共用 `chat-area-wrap + chat-status-bar`,未来加 Telegram/Feishu 也用同样模板,跟 ChatView 一起构成"5 种 view 同一套外壳"

#### E2E 更新

- `e2e/wechat-channel.spec.ts`:"返回聊天" 按钮从 `.back-btn` 改成 `.chat-status-action`

#### 验证

- `npm run lint` 0 error
- `tsc --noEmit` 0 error
- `npm run test:unit` 14/14 全过
- `pytest --ignore=tests/test_e2e_features.py` 926/936 过(同前,10 e2e 因 Nexus.app 占端口)
- `ruff check nexus/` All checks passed(本次未改 backend)
- preview 截图(1280×800 桌面窗口):
  - light mode Settings:顶部 36px 顶条"设置 / ← 返回聊天",下方 5 行扁平列表,右侧 toggle,无卡片
  - dark mode Wechat:整窗森林绿 canvas,主区 paper 浮起,顶端 36px "微信通道 / ← 返回聊天" 顶条,brand copy 单列 + ChannelViewBase bind card
  - light mode Wechat:整窗白底 paper,跟 ChatView 顶部条形态一致

---

### [1.2.0] — 2026-07-16 第九轮 UI 重设计 + 模拟真人 E2E

#### Changed — 顶栏三件套 (ModelSwitcher + ThemeToggle + ChatView 排版)

第八轮把 SettingsView/WechatAssistantView/SetupView 三子视图对齐到 ChatView 同源
`chat-area-wrap + chat-status-bar` 模板,本轮把 ChatView 顶栏本身的右侧从单一
"本地在线" pill 扩成 Claude Desktop 同款三件套:

- **ModelSwitcher**(`.model-switcher-chip`):点开 dropdown 切 active model;`.chat-status-actions` 内右侧;
  空 models=[] 时降级只显示 chip,绝不渲染空 dropdown。7 单测覆盖 store 联动 + 外部 click 收起。
- **ThemeToggle**(`.theme-toggle`):☀️ / 🌙 emoji 跟随 store.darkMode;`aria-pressed` 同步。5 单测覆盖图标 / 状态 / store toggle。
- **ChatView 排版**:`.chat-status-bar` 左标题(`currentConv.title || '新任务'`)/ 右 `.chat-status-actions`(pill + chip + theme);
  36px 高、`data-tauri-drag-region` 让 macOS chrome drag 区跨整条;`flex` 横向排,`gap: 8px`。

WHY:Claude Desktop / Cursor / ChatGPT 三家右侧都是"status + 切换 + 设置"三件套,
一个孤立 pill 显得简陋。`data-tauri-drag-region` 跨整条顶栏让用户抓标题区也能拖动
窗口,不只是左侧空白。

#### Added — E2E 模拟真人 journey-redesign 套件 (10 个 journey)

按 SPEC §4.1 端到端覆盖主流 agent 操作习惯:`e2e/journey/journey-redesign.spec.ts`:

1. **empty-state-hero**        — 删全部会话 → hero + 4 prompt + 大输入框
2. **new-conversation-flow**   — 空态输入"你好" → Enter → 助手回复
3. **multi-turn-context**      — 第 2 轮带前轮上下文(assistant 引用上轮"茶馆")
4. **tool-call-visible**       — 触发 shell_run → ToolCallCard 透明化 (mock)
5. **thinking-collapsed**      — 思考块折叠 + 点开看细节
6. **sidebar-search**          — 多会话 → 搜索 → 过滤命中
7. **model-switch**            — 顶栏 chip 点开 dropdown
8. **theme-toggle**            — 切深色 + 验证 data-theme
9. **wechat-bind-modal**       — 顶栏微信按钮 → QR 弹窗
10. **stop-during-stream**     — 流期间点 stop → "[已停止]" marker (mock)

`helpers.ts` 微调:`sendButton` 返回 `.or()`,strict mode 多匹配由调用方 `.first()` 处理。
j4/j10 需 `NEXUS_E2E_MOCK=1` 跑(mock 强制固定 tool_call + 流延迟 2s),默认实跑
8 passed + 2 skipped。

#### Added — 整体产品级打磨 · 第八轮计划 #213 (第十一轮 2026-07-16)

按用户反馈"完美的产品 (整体产品级打磨) 持续" 一次性扫尾 8 个 atomic
sub-task(焦点环 / 快捷键 / scroll affordance / loading 节奏 / hover / a11y /
微信 footer / form focus),所有改动纯前端,后端 0 触碰。

1. **全局焦点环 (Task 1, commit `524c48b`)**
   - `tokens.css` 新增 `--focus-ring` / `--focus-ring-offset` 双 token,
     浅色 = 纯黑 #000 + 白 offset,深色 = 纯白 #fff + #1f1f1f offset
   - `shell.css` 全局 `*:focus-visible` 规则 `outline: 2px solid var(--focus-ring);
     outline-offset: 2px`,键盘用户一眼可见焦点位置

2. **全局键盘快捷键 (Task 2, commit `bdb3667`)**
   - `useGlobalShortcuts.ts` 新 hook,顶层 `keydown` 监听 + `metaKey||ctrlKey`
     跨平台(Cmd macOS / Ctrl Win/Linux)
   - `Cmd+N` 新建对话,`Cmd+K` 聚焦 sidebar 搜索框,`Cmd+/` 聚焦 composer,
     `Esc` 关最上层 modal(ModelConfig / WechatPlugin)
   - 4 个测试覆盖 4 个快捷键路径

3. **chat scroll affordance (Task 3, commit `44afb63`)**
   - `chat.css` `.chat-scroll` 加双层 `background-attachment: local` 渐隐边
     视觉魔术:顶部到底渐变(可滚)+ radial shadow,滚到底渐隐自动消失

4. **loading-dot 三段 delay (Task 4, commit `44afb63`)**
   - `.loading-dot:nth-child(1/2/3)` 显式 `animation-delay: 0/150/300ms`,
     跳点节奏感

5. **prompt-card hover 视觉 (Task 5, commit `44afb63`)**
   - `.prompt-card:hover` 加 `translateY(-1px)` + box-shadow 阴影强化(锁测试
     补一条确保不回归)

6. **a11y 补全 (Task 6, commit `4ab7050`)**
   - `Sidebar.tsx` `btn-new-task` 加 `aria-label="新建对话 (快捷键 Cmd+N / Ctrl+N)"`
   - `recent-panel` 容器加 `aria-live="polite" aria-relevant="additions text"`
   - `SettingsView.tsx` 思考过程 / 深色模式 toggle 加 `aria-pressed` + 动态
     `aria-label` 描述当前状态
   - `ChatArea/index.tsx` `ErrorBanner` 外层 `<div role="status" aria-live="assertive">`,
     错误即时播报

7. **Wechat footer 双色态视觉 (Task 7, commit `44afb63`)**
   - `shell.css` `.footer-link--wechat.is-connected` 图标 + status 全部走
     `--wechat` 绿,未连接态保持 sidebar-fg-secondary + dashed border

8. **form input focus 统一 + hint.is-error 边条 (Task 8, commit `44afb63`)**
   - `views.css` `setup-card / settings-list / setting-row / wechat-bind-card`
     全部 `input:focus-visible` → forest 边框 + 3px rgba(74,124,89,0.20) 光晕
   - `.hint.is-error` 加 `border-left: 3px solid #c05443` 红边条 + 浅红底

**关键文件**

- 新增:`frontend/src/lib/hooks/useGlobalShortcuts.ts` + 4 个锁测试文件
  (focus-ring / useGlobalShortcuts / product-polish / a11y-polish)
- 修改:`tokens.css` / `shell.css` / `chat.css` / `views.css` /
  `Sidebar.tsx` / `SettingsView.tsx` / `DesktopShell.tsx` / `ChatArea/index.tsx`
- 锁测试总计:**4 个测试文件 / 24 个 case**,覆盖契约
  - Task 1 焦点环 token × 4 + 全局规则 × 2
  - Task 2 快捷键 4 path
  - Task 3 chat-scroll 渐隐背景 + local 附件
  - Task 4 loading-dot 三段 delay
  - Task 5 prompt-card hover 视觉
  - Task 6 a11y 4 条(aria-label / aria-live / aria-pressed × 2 / role=status)
  - Task 7 footer-link is-connected 双色
  - Task 8 form input focus × 5 + hint.is-error 边条

**验证**

- `npx vitest run` 26 个文件 / **186 个测试**全过(基线 162 + 新 24)
- `npx tsc -b` 0 error
- `npm run lint` 0 error
- `ruff check nexus/` All checks passed
- browser preview(vite dev port 30088)实测契约:
  - `btn-new-task.aria-label` = "新建对话 (快捷键 Cmd+N / Ctrl+N)" ✓
  - `recent-panel.aria-live` = "polite" + `aria-relevant` = "additions text" ✓
  - `footer-link--wechat.aria-label` = "微信通道 未绑定" ✓
  - Cmd+K 触发 `BODY → INPUT:search` focus 切换 ✓
  - SettingsView 渲染 5 个 setting-row,思考过程 / 深色模式 toggle 双
    aria-pressed + aria-label 正确
- **DMG**:重打 1.2.0 (`release/Nexus-1.2.0-arm64.dmg`,86 MB,Tauri 2 + sidecar)

#### 验证

- `npm run lint` 0 error
- `npm run test:vitest` 155/155 PASS
- `./node_modules/.bin/playwright test e2e/journey/journey-redesign.spec.ts` 8 passed + 2 skipped (3.1 min)
- `ruff check nexus/` All checks passed(本次未改 backend)
- **DMG:** `release/Nexus-1.2.0-arm64.dmg`(~70 MB,Tauri 2 + sidecar)
