...
2. 跑 `python scripts/eval_rubrics.py --samples ~/.nexus/evaluations/rubric_eval_samples.jsonl --output ~/.nexus/evaluations/eval_report.json`
3. 看 report：
...
`~/.nexus/evaluations/eval_report.json` 当前的真环境验收结果（commit `7ea9cbe` 之后）：
...
12 条人工样本（`~/.nexus/evaluations/rubric_eval_samples.jsonl`）覆盖：
...
.venv/bin/python scripts/eval_rubrics.py \
    --samples ~/.nexus/evaluations/rubric_eval_samples.jsonl  # meta-eval
```
...
- 人工样本：`~/.nexus/evaluations/rubric_eval_samples.jsonl`
- 当前 meta-eval 结果：`~/.nexus/evaluations/eval_report.json`
