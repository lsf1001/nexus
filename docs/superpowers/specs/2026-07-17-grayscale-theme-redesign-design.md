...
| `frontend/src/components/desktop/styles/views.css` | 引用 `forest / forest-2 / forest-soft / sage / tea / wechat` 的位置全部改灰阶 token |
| `frontend/src/components/desktop/styles/shell.css` | 同上 |
| `frontend/src/components/desktop/styles/chat.css` | 同上 |
| `frontend/src/components/desktop/styles/responsive.css` | 响应式断点内如有硬编码彩色,改 token |
...
| `--faint`(三级文字) | `--ink-3` |
| `--canvas`(背景起始) | `--paper-2` |
| `--paper-soft`(卡片底) | `--paper-2` |
| `--paper-tint`(渐变止) | `--paper-3` |
| `--line`(描边) | `--line` |
| `--line-strong`(强描边) | `--line-2` |
| `--forest`(强调) | `--accent` |
| `--forest-2`(强调 hover) | `--ink`(light) / `--accent-soft`(dark) |
| `--forest-soft`(chip 底) | `--accent-soft` |
